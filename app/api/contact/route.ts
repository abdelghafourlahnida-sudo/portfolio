...
export async function POST(req: NextRequest) {
  try {
    ...
    // }
  console.log("dddddddddddddddd 11111111111");
    // ── Field validation ─────────────────────────────────────────────────
    if (!name?.trim() || !email?.trim() || !subject?.trim() || !message?.trim()) {
      return NextResponse.json({ error: 'All fields are required.' }, { status: 400 })
    }
  console.log("dddddddddddddddd 222");
  ...
  console.log("dddddddddddddddd 55");
...
console.log('SMTP server is ready')
    ...
    const info = await transporter.sendMail({
      from: `"abdelghafouur Portfolio" <${smtpUser}>`,
      to: receiver,
      replyTo: `"${name}" <${email}>`,
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#111827;border-radius:16px;border:1px solid rgba(93,202,165,0.15);overflow:hidden;">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#5DCAA5,#38866e);padding:28px 32px;">
              ...
              <p style="margin:6px 0 0;color:rgba(10,15,30,0.7);font-size:13px;">via Abdelghafouur contact form</p>
            </td>
          </tr>
          <!-- Body -->
          <tr>
          ...
          <tr>
            <td style="padding:16px 32px;border-top:1px solid rgba(255,255,255,0.06);text-align:center;">
              <p style="margin:0;color:#4B5563;font-size:11px;">Sent via Abdelghafour Lahnida's Portfolio</p>
            </td>
          </tr>
        </table>
      </td>
